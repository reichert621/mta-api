...
def main():
    ...
    with open(args.stations_file, "r") as f:
        ...
        for row in reader:
            if row["parent_id"] in stations:
                stations[row["parent_id"]]["stops"][row["stop_id"]] = [
                    float(row["lat"]),
                    float(row["lon"]),
                ]
                ...
                stations[row["parent_id"]] = {
                    "name": set([row["name"]]),
                    "stops": {row["stop_id"]: [float(row["lat"]), float(row["lon"])]},
                }
    ...
    for station in stations.values():
        ...
        station["id"] = md5(
            "".join(station["stops"].keys()).encode("utf-8")
        ).hexdigest()[:ID_LENGTH]
    ...
    json.dump(
        keyed_stations, sys.stdout, sort_keys=True, indent=4, separators=(",", ": ")
    )
